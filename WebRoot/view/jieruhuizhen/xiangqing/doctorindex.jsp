							...
							<fieldset id="menu_func_div" class="layui-elem-field">
								...
								<div class="layui-form-item layui-form-text">
									...
									<div class="layui-input-block">
										...
										<div class="layui-form-item">
											<button type="button" class="layui-btn layui-btn-normal" lay-submit="" lay-filter="demo1">
												<i class="layui-icon">&#xe6ed;</i>创建会议
											</button>
											<button type="button" class="layui-btn layui-btn-normal" lay-submit="" lay-filter="demo2">
												<i class="layui-icon">&#xe6ed;</i>编辑会议
											</button>
											<button type="button" class="layui-btn layui-btn-normal" id="delIntervention">
...
base : "../js/"
		...
		form.on('submit(demo1)', function(data){
			...
			$.ajax({
		          url : '../intervention/save.do?interventionKey='+interventionKey,
		          type : 'post',
		          async: false,
		...
		form.on('submit(demo2)', function(data){
			if("${isEdit}"=='false'){
				common.cmsLaySucMsg("距离会诊开始，已经很近了或者正在进行，不能再修改会诊信息了。");
				return false;
			}
			var userSaveLoading = top.layer.msg('数据提交中，请稍候',{icon: 16,time:false,shade:0.8});
			data.field.member = member;
			$.ajax({
		          url : '../intervention/updateById.do?interventionKey='+interventionKey,
		          type : 'post',
		          async: false,
		          data :{"json":JSON.stringify(data.field)},
		          success : function(data) {
		 ...
		 $("#joinReadImage").click(function(){
		   ...
		   $("#delIntervention").click(function(){
			  if(interventionKey== undefined ){
				  common.cmsLayErrorMsg("请先选择要删除的会诊！");
				  return;
			  }
			  top.layer.confirm("是否删除会诊", {
			        title: "提示",
			        resize: false,
			        btn: ['确定', '取消'],
			        btnAlign: 'c',
					...
					$.ajax({
			          url : "../intervention/delByinterventionKey.do?interventionKey="+interventionKey,
			          type : 'GET',
			          async: false,
			          success : function(data) {
			        	  data = eval("("+data+")");
