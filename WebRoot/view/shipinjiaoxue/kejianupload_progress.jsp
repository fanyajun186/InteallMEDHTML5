...
<body class="body_bg">
    <div class="layui-col-md12" style="margin-top: 10px">
             <input hidden="hidden" name="videoKey" value="${videoKey}">
...
$(function(){
    ...
    $('#fileupload').bind('fileuploadsubmit', function (e, data) {
        data.formData = { "random": random,"stuuid":stuuid };  //如果需要额外添加参数可以在这里添加
    });
	$("#fileupload").fileupload({
		...
		add: function(e, data) { //add表示在选择文件时判断格式是否正确
			...
			if(data.result.code==0){
            	...
            	$('#'+data.result.random).html("<span style='color:red'>上传失败<span>");
                response = false;
            }else if(data.result.code==2){
            	$('#'+data.result.random).html("<span style='color:red'>文件已存在<span>");
                 response = false;
            }
		},
