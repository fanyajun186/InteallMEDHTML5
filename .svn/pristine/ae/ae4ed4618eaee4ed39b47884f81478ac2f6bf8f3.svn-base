...
function submitData(){
		...
		var keyWord=$("#keyWord").val();
		 if(title==null || title==''){
			...
			$.post("${pageContext.request.contextPath}/zhishikuManage/save.do",{'id':'${id}','title':title,'content':content,'zhishikuType.id':typeId,'contentNoTag':UE.getEditor('editor').getContentTxt(),'summary':UE.getEditor('editor').getContentTxt().substr(0,155),'keyWord':keyWord},function(result){
				if(result.success){
					alert("知识库修改成功！");
					resetValue();
				}else{
...
<div id="p" class="layui-fluid" title="编写知识库" style="padding: 10px">
	<form class="layui-form layui-col-md12">
        ...
        <div class="layui-form-item layui-form-text">
          <label class="layui-form-label"></label>
           <div class=" layui-col-md10">
           		<button class="layui-btn layui-btn-normal" type="button" onclick="submitData()">修改知识库</button>
           </div>
        </div>
	</form>
 </div>
 ...
 ue.addListener("ready",function(){
     //通过ajax请求数据
     UE.ajax.request("${pageContext.request.contextPath}/zhishikuManage/findById.do",
         {
             method:"post",
             async : false,  
             data:{"id":"${id}"},
             onsuccess:function(result){
    			...
    			var parentId=result.zhishikuType.parentId;
    			var id=result.zhishikuType.id;
    	     	$("#classification option[value='"+parentId+"']").attr("selected","selected");
    	     	if(parentId!=""){
    				var url="<%=request.getContextPath()%>/zhishikuManage/findBySubclass.do";
    				var args={"isParent":parentId};
    				$.getJSON(url,args,function(data){
    						for(var i=0;i<data.length;i++){
    							var Subclassification_id=data[i].id;
    							var Subclassification_name=data[i].typeName;
    							$("#Subclassification").append("<option value='"+Subclassification_id+"'>"+Subclassification_name+"</option>");
    	     					$("#Subclassification option[value='"+id+"']").attr("selected","selected");
    						}				
    				});
    			}
             }
