			...
			<div class="layui-col-md9">
				...
				<fieldset id="menu_func_div" class="layui-elem-field">
                    ...
                    <div class="layui-form-item layui-col-md12" id="toolButton"
                        ...
                        <button class="layui-btn layui-btn-normal" id="submitReview">提交</button>
                    </div>
...
<script type="text/javascript">
    ...
    function onClick(e,treeId, treeNode) {
        ...
        clicked_type = true;
        //检查所见内容
        var jianchasuojianText = $("#jianchasuojian").next("div").find("iframe").contents().find("body").html();
        //诊断结果内容
        var zhenduanjieguoText = $("#zhenduanjieguo").next("div").find("iframe").contents().find("body").html();
        if(!treeNode.isParent){
            ...
            $.ajax({
                ...
                success:function(data){
                   ...
                   //建立编辑器（检查所见）
                   var checkView = jianchasuojianText+data.ReportTemplate.checkView;
                   ...
                   //建立编辑器（诊断结果）
                   var diagnosisResult = zhenduanjieguoText+data.ReportTemplate.diagnosisResult;
	...
	layui.config({
		...
		if('${consultationWithBLOBs.primaryAuditLogin}'!=''){
			...
			$("#newDiv").show();
			chushenrenLogin = "${consultationWithBLOBs.primaryAuditLogin}";
			chushenrenName = "${consultationWithBLOBs.primaryAuditName}";
            fushenrenLogin = "${consultationWithBLOBs.ultimateAuditLogin}";
			fushenrenName = "${consultationWithBLOBs.ultimateAuditName}";
            shifoushenhe = true;
		}
	...
	form.on('switch(switchTest)', function(data) {
		if (this.checked == true) {
			...
			$("#newDiv").hide();
			shifoushenhe = false;
			chushenrenLogin=undefined;
			chushenrenName=undefined;
			fushenrenLogin=undefined;
			fushenrenName=undefined;
			$("#chushenren").val("");
			$("#fushenren").val("");
		}
	});
	...
	form.on('submit(demo1)', function(data){
		if(shifoushenhe==true){
			if(chushenrenLogin==undefined||chushenrenName==undefined){
	            common.cmsLayErrorMsg("已经要求审核，请先选择审核人，在保存。");
	            return false;
	        }
		}
        ...
        if(shifoushenhe==false){
        	chushenrenLogin="";
            chushenrenName="";
        	fushenrenLogin="";
            fushenrenName="";
        }
        if(fushenrenLogin==undefined){
        	fushenrenLogin="";
        	fushenrenName="";
        }
        data.field.chushenrenLogin = chushenrenLogin;
        data.field.chushenrenName = chushenrenName;
        data.field.fushenrenLogin = fushenrenLogin;
        data.field.fushenrenName = fushenrenName;
        data.field.shifoushenhe = shifoushenhe;
        data.field.caseId = "${consultationWithBLOBs.caseId}";
        data.field.reportId = reportId;
        if(shifoushenhe==false){
        	top.layer.confirm("没有指定审核人，是否结束会诊", {
      	        title: "提示",
      	        resize: false,
      	        btn: ['确定','取消'],
      	        btnAlign: 'c',
    ...
    $("#chushenren").click(function(){
    	//var text = $(this).val();
    	//if(text==null || text==''){
    		var url = "redirectShenheren.do?type=1";
            common.cmsLayOpen('选择审核人',url,'700px','450px');
    	//}
    	
    })
    $("#fushenren").click(function(){
    	//var text = $(this).val();
    	//if(text==null || text==''){
        	var url = "redirectShenheren.do?type=2";
        	common.cmsLayOpen('选择审核人',url,'700px','450px');
    	//}	
    })
    $("#submitReview").click(function(){
        var caseId = "${consultationWithBLOBs.caseId}";
        if(reportId==""||reportId==undefined||reportId==null){
        	common.cmsLayErrorMsg("请先保存报告，在提交审核。");
            return;
        }
        if(chushenrenLogin==undefined||chushenrenName==undefined){
            common.cmsLayErrorMsg("请选择审核人，在提交审核。");
            return;
        }
        $.ajax({
            url:"submitReview.do?caseId="+caseId+"&reportId="+reportId+"&stuuid="+stuuid,
            type:"get",
            success:function(data){
                data = eval("("+data+")");
                if(data.code==0){
                	common.cmsLaySucMsg("报告提交成功！")
                	$("#toolButton").hide();
    ...
    $(document).ready(function(){
        ...
        if("${consultationWithBLOBs.state}" ==='3'||"${consultationWithBLOBs.state}"==='4'||"${consultationWithBLOBs.state}"==='5'){
        	$("#toolButton").show();
        }
    });
