		...
		init : function(options, swfUploadOptions) {
			...
			return this.each(function() {
				...
				var settings = $.extend({
					...
					queueSizeLimit  : 9999,                // The maximum number of files that can be in the queue at one time
		...
		onSelect : function(file) {
			...
			for (var n in this.queueData.files) {
				...
				if (queuedFile.uploaded != true && queuedFile.name == file.name) {
					var replaceQueueItem = confirm('文件名为 "' + file.name + '" 已存在上传队列中\n您想替换当前队列中文件吗?');
					if (!replaceQueueItem) {
...
//			 fileSize += Math.round(file.size /1024000);
...
//			}
			...
			if (settings.itemTemplate == false) {
				settings.itemTemplate = '<div id="${fileID}" class="uploadify-queue-item" style="display:none">\
					<div class="cancel">\
						<a href="javascript:$(\'#${instanceID}\').uploadify(\'cancel\', \'*\')">X</a>\
					</div>\
			...
			if (settings.checkExisting) {
				$.ajax({
					...
					success : function(data) {
						if (data == 1) {
							var overwrite = confirm('文件名为 "' + file.name + '" 已存在于服务器上.\n你想替换现有的文件吗?');
							if (!overwrite) {
								this.cancelUpload(file.id);
								$('#' + file.id).remove();
